...
class FeatureScheduleViewSet(viewsets.ModelViewSet):  # type: ignore[type-arg]
    ...
    permission_classes = [IsAuthenticated, NestedEnvironmentPermissions]
    def get_queryset(self):  # type: ignore[no-untyped-def]
        if getattr(self, "swagger_fake_view", False): return FeatureSchedule.objects.none()
        environment_api_key = self.kwargs.get("environment_api_key")
        feature_pk = self.kwargs.get("feature_pk")
        if environment_api_key and feature_pk:
            environment = get_object_or_404(Environment, api_key=environment_api_key)
            return FeatureSchedule.objects.filter(environment=environment, feature_id=feature_pk).select_related("feature", "environment", "created_by")
        return FeatureSchedule.objects.none()
    def perform_create(self, serializer):  # type: ignore[no-untyped-def]
        environment_api_key = self.kwargs.get("environment_api_key")
        feature_pk = self.kwargs.get("feature_pk")
        environment = get_object_or_404(Environment, api_key=environment_api_key)
        feature = get_object_or_404(Feature, pk=feature_pk, project=environment.project)
        serializer.save(feature=feature, environment=environment, created_by=self.request.user)
    @action(detail=False, methods=["GET"], url_path="by-environment")
    def by_environment(self, request, *args, **kwargs):  # type: ignore[no-untyped-def]
        environment_api_key = self.kwargs.get("environment_api_key")
        environment = get_object_or_404(Environment, api_key=environment_api_key)
        schedules = FeatureSchedule.objects.filter(environment=environment, status='pending').select_related("feature", "environment", "created_by").order_by("scheduled_at")
